...
- Asymmetric sheet-failure handling: on a *successful* account, a failed sheet append prints `WARNING: Sheet log failed: ...` (:540-541); on a *failed* account it is swallowed silently (:549-552). The JSONL row exists either way — the local log is the audit trail of record.
